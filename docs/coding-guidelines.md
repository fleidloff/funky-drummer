...
**A rule here names the file that motivated it, and none of them do yet.** Every
rule below arrived carried over from a sibling project rather than grown here.
That is a debt, not a style: the first time a rule meets a real file in this
...
- *lint-enforced* — `npm run lint` fails on a violation. The rule that does most
  ...
  `eslint.zones.mjs`; see [Enforcement](#enforcement).
...
*human-checked* — worth a structural test that reads the folder set from disk,
written with the first component.
...
*human-checked* — worth a structural test case: no component imports a
specifier that climbs out of its own folder.
...
*human-checked* — worth a structural test case: no `index.ts` anywhere under
`src/components/`.
...
*lint-enforced* (zone 2), plus a structural test for the mock case — it reads
every route file from disk and matches `from`, `vi.mock`, dynamic `import()` and
`require()` specifiers in any quote style. Such a guard excludes exactly one
file, itself, because its own fixtures are deliberate violations; a second
exclusion should fail its count assertion.
...
*lint-enforced* (`funky-drummer/no-styling-in-features` in
`eslint.config.mjs`), with a config test proving it fires — see
...
actually does rather than after a generic layer cake — for a groove box that is
likely to mean a folder for the transport and scheduling rules, one for the
groove and how it is performed, one for the kit and its samples, one for what is
stored, and one for turning state into what the UI says.
...
into one concern folder climbed while nothing was watching. What eventually
guarded it was one folder's worth of its fan-in — the door rule under
...
`funky-drummer/import-boundaries`, which is `import/no-restricted-paths` and does
most of the work below, and `funky-drummer/no-styling-in-features`, which is
...
absence looks exactly like success. A config test is what stands between us and
that.
...
and nothing can fire it. Assert it at the generator instead, by calling the zone
...
and they take the next free numbers. That section is empty, so zone 6 is the
only intra-slice one.
...
`funky-drummer/no-styling-in-features` is the one block that carries a `files` key,
...
trip it because there is no JSX in one. Like the zones, it is proven in a config
test rather than believed: a fixture that violates it cannot be committed, so
the Node API is the only place it is watched to reject one.
