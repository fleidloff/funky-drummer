...
`docs/architecture.md` § *The arrows inside a slice* draws that graph. It is
empty today, so no such zone exists yet and importing a sibling concern directly
is correct.
...
**A test of the drummer's variation pins a seed.** The performance is generated
fresh every bar, so an assertion that does not fix the seed is asserting one
sample of a distribution and will go red on its own schedule. Pin the seed and
assert what that seed produces, or assert the bound the variation must stay
inside — never the shape of one unseeded run.
