...
3. a **reference** — an ADR, a spec folder or a ticket, where the code would
   otherwise read as arbitrary. One line: `// ADR 0004`, not a summary of it
...
folders; that section is empty, so zone 6 is the only intra-slice one.
...
is, never for where it is used: `Button`, not `PlayButton`; `Card`, not
...
- **Randomness comes from a seed your unit was given**, never from a bare
  `Math.random`. The drummer varies every bar, and a variation nobody can
  reproduce is a bug nobody can reproduce either.
