...
6. **Write the report to `specs/N-title/.verify/report.md`.** That path is
   gitignored scratch even though the folder around it is tracked (see the repo's
   `.gitignore`), so your report is working material rather than the record. What survives is the row
...
finding in its own right. While that section is empty, neither applies.
