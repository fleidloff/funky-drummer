...
read it before specifying work inside a feature. It is empty today, and stays
empty until a slice has grown enough concerns to need a map. **A concern folder's `index.ts`
...
This app is a drummer, so scheduling accuracy is a first-class requirement rather
