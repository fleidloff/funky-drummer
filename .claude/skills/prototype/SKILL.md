...
- **`docs/concept/Project.md`.** *Core experience*, *Controls* and *Look* decide
  the frame width, the tap targets and what has to be visible without scrolling —
  a portrait phone held in one hand, the other hand on the instrument.
  `docs/concept/app.jpg` is the mockup it refers to; look at it.
- **`src/app/globals.css`.** The `@theme` block is the palette, the radii and
  the shadow. Copy the values; do not invent a second set. **It does not exist
  yet** — until it does, pick the palette from `docs/concept/app.jpg` and say in
  the report that the tokens are invented rather than matched, so nobody reads
  the drawing as agreeing with a theme that has not been written.
- **The nearest existing components**, for shapes you are reusing. Read them to
  match the look. Never import them.

Where the drawing has to invent something the requirements left silent *and* the
invention is the user's business — what sits above the fold, whether a control is
there at all, what the copy calls a mode — it goes in the invented-list of §7
rather than being settled by taste.
...
- **No audio.** A play button toggles a visible running state and nothing else.
  Sound is the one thing a prototype can't help with, and wiring up a real kit
  buys a demo the spec didn't ask for — for this app that is the sharpest version
  of the rule, because the thing it *is* cannot be drawn. Say so in the report
