...
- anything `docs/concept/Project.md` already settles — what the app is, who it is
  for, and what is explicitly out of scope.
...
### Check the concept before asking a product question

**Read [docs/concept/Project.md](../../../docs/concept/Project.md) before any
question that turns on what the person using this would do** — what they want,
what loses them, what is deliberately not being built. Its *Who it is for* and
*Out of scope* sections settle more of these than they look like they do, and a
question that section already answers is one you should not be spending.

Where it is silent, say so in the question rather than deciding by taste: name
that the concept does not cover this, and let the user settle it. That answer is
worth writing down, because it is the kind of thing the concept should have said.
...
question about build order rather than about the product.
...
its reason. An implementation fork rarely turns on the product, and a question
that does was a `spec.md` question that arrived late.
